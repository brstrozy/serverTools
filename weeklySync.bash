...
inputFile="paths.txt"
LOG_DIR="/mnt/user/logs/weeklySync/"`date +"%m-%d-%Y"`""
...
for path in ${paths[@]}; do 
    ...
    if [ $? -ne 0 ]; then
        HAD_ERROR=$?
        /usr/local/emhttp/webGui/scripts/notify -e "Weekly Sync Status" -i alert -s "Weekly Sync Error!!!" -d "Error: rsync exited with error code $? while backup up ${pathArray[0]}"
    fi
...
if [ $HAD_ERROR -eq 0 ]; then
	/usr/local/emhttp/webGui/scripts/notify -e "Weekly Sync Status" -i normal -s "Weekly Sync Complete!"
fi
